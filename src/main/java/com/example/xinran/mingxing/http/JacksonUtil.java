...
public class JacksonUtil {
	...
	static{
		//allow empty object to be serialized into {}
		OBJECT_MAPPER.configure(SerializationFeature.FAIL_ON_EMPTY_BEANS, false);
//		OBJECT_MAPPER.configure(DeserializationFeature.EAGER_DESERIALIZER_FETCH, true);
	}
    ...
    public static <T,R> T unwrapValue(String fromValue, Class<T> wrapType,Class<R> rawType){
		TypeFactory typeFactory =OBJECT_MAPPER.getTypeFactory();
		JavaType javaType=typeFactory.constructParametricType(wrapType, rawType);
		try {
			return OBJECT_MAPPER.readValue(fromValue,javaType);
		} catch (JsonProcessingException e) {
			   throw new IllegalArgumentException("The given string value: "
		                + fromValue + " cannot be transformed to " + wrapType + ". Detail message : " + e.getMessage());
		}
    }
    
    public static void main(String[] args) {
    	String x="{\"msg\":\"\\\"title\\\":\\\"<font color=\\\\\"#000000\\\\\"></font>Notify\\\"\"}";
    	JacksonUtil.convertValue(x, Map.class);
//    	BaseLogInfo base=new InterfaceLogInfo();
//    	System.out.println(toString(base));
//    	CommonResponse rs=new CommonResponse();
//    	rs.setData(base);
//    	rs.setCode(200);
//    	String s= toString(rs);
//    	System.out.println(s);
//    	CommonResponse x=unwrapValue(s,CommonResponse.class,InterfaceLogInfo.class);
//    	System.out.println(InterfaceLogInfo.class.cast(x.getData()).getDataType());
    }
}
