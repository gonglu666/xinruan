...
//        }
    ...
    private static void handleDatas(Map<String,Map<String,String>> result) {
        ...
        //开始删除部门
        for (Integer num : depart) {
            HashMap<String,String> departMap = departLevel.get(num);
            Iterator<String> departIterator = departMap.keySet().iterator();
            while(departIterator.hasNext()) {
                String next = departIterator.next();
                deleteDepart(next);
            }

        }

        System.out.println("-------------------------部门删除完成------------------------");
        //删除公司
        for (Integer num : unit) {
            HashMap<String,String> unitMap = unitLevel.get(num);
            Iterator<String> unitIterator = unitMap.keySet().iterator();
            while(unitIterator.hasNext()) {
                String next = unitIterator.next();
                deleteUnit(next);
            }
        }

        System.out.println("-------------------------公司删除完成------------------------");
    ...
    private static void deleteDepart(String departCode) {
        try {
            String users = HttpUtils.sendByJson(Headers.of(headers), getUserFDepart + codeId.get(departCode), JacksonUtil.toString(null), HttpMethod.GET);
            List<Map> list = JacksonUtil.convertList(users, Map.class);
            while (list.size()>0) {
                list.forEach(map ->{
                    HttpUtils.sendByJson(Headers.of(headers), deleteUser + map.get("id"), JacksonUtil.toString(null), HttpMethod.DELETE);
                });
                users = HttpUtils.sendByJson(Headers.of(headers), getUserFDepart + codeId.get(departCode), JacksonUtil.toString(null), HttpMethod.GET);
                list = JacksonUtil.convertList(users, Map.class);
            }
            HttpUtils.sendByJson(Headers.of(headers), deleteDepart + departCode, JacksonUtil.toString(null), HttpMethod.DELETE);
        } catch (Exception e) {
            System.out.println("部门删除失败："+departCode);
        }
    }

    private static void deleteUnit(String unitCode) {
        try {
            String positions = HttpUtils.sendByJson(Headers.of(headers), getPosiFUnit + codeId.get(unitCode), JacksonUtil.toString(null), HttpMethod.GET);
            List<Map> list1 = JacksonUtil.convertList(positions, Map.class);
            while (list1.size()>0) {
                list1.forEach(map ->{
                    HttpUtils.sendByJson(Headers.of(headers), deletePosition + map.get("id"), JacksonUtil.toString(null), HttpMethod.DELETE);
                });
                positions = HttpUtils.sendByJson(Headers.of(headers), getPosiFUnit + codeId.get(unitCode), JacksonUtil.toString(null), HttpMethod.GET);
                list1 = JacksonUtil.convertList(positions, Map.class);
            }

            String duties = HttpUtils.sendByJson(Headers.of(headers), getDutyFUnit + codeId.get(unitCode), JacksonUtil.toString(null), HttpMethod.GET);
            List<Map> list2 = JacksonUtil.convertList(duties, Map.class);
            list2.forEach(map ->{
                HttpUtils.sendByJson(Headers.of(headers), deleteDuty + map.get("id"), JacksonUtil.toString(null), HttpMethod.DELETE);
            });
            HttpUtils.sendByJson(Headers.of(headers), deleteCompany + codeId.get(unitCode), JacksonUtil.toString(null), HttpMethod.DELETE);
        } catch (Exception e) {
            System.out.println("公司删除失败："+unitCode);
        }
